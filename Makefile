...
docker-compose-spark-submit: ## Run spark-submit from within local docker containerized infrastructure (which must be running first). Set params with django_command="..."
	...
	spark-submit \
	--driver-memory "2g" \
	--packages org.postgresql:postgresql:42.2.23,io.delta:delta-spark_2.12:3.0.0,org.apache.hadoop:hadoop-aws:3.3.1,org.apache.spark:spark-hive_2.12:3.2.1 \
	${if ${python_script}, \
		${python_script}, \
...
pyspark-shell: ## Launch a local pyspark REPL shell with all of the packages and spark config pre-set
	...
	--packages org.postgresql:postgresql:42.2.23,io.delta:delta-spark_2.12:3.0.0,org.apache.hadoop:hadoop-aws:3.3.1 \
