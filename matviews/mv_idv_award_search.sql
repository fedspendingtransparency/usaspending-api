...
CREATE MATERIALIZED VIEW mv_idv_award_search_temp AS
SELECT
  ...
  COALESCE(rl_county_lookup.county_name, transaction_fpds.legal_entity_county_name) AS recipient_location_county_name,
  ...
  COALESCE(pop_county_lookup.county_name, transaction_fpds.place_of_perform_county_na) AS pop_county_name,
  ...
  NULL::text AS cfda_program_title,
...
LEFT OUTER JOIN
   ref_country_code AS pop_country_lookup on (
      pop_country_lookup.country_code = COALESCE(transaction_fpds.place_of_perform_country_c, 'USA')
      OR pop_country_lookup.country_name = transaction_fpds.place_of_perform_country_c)
LEFT OUTER JOIN
   ref_country_code AS rl_country_lookup on (
      rl_country_lookup.country_code = COALESCE(transaction_fpds.legal_entity_country_code, 'USA')
      OR rl_country_lookup.country_name = transaction_fpds.legal_entity_country_code)
LEFT OUTER JOIN
 (SELECT DISTINCT ON (state_alpha, county_numeric) state_alpha, county_numeric, UPPER(county_name) AS county_name FROM ref_city_county_state_code) AS rl_county_lookup on
   rl_county_lookup.state_alpha = transaction_fpds.legal_entity_state_code and
   rl_county_lookup.county_numeric = LPAD(CAST(CAST((REGEXP_MATCH(transaction_fpds.legal_entity_county_code, '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 3, '0')
LEFT OUTER JOIN
 (SELECT DISTINCT ON (state_alpha, county_numeric) state_alpha, county_numeric, UPPER(county_name) AS county_name FROM ref_city_county_state_code) AS pop_county_lookup on
   pop_county_lookup.state_alpha = transaction_fpds.place_of_performance_state and
   pop_county_lookup.county_numeric = LPAD(CAST(CAST((REGEXP_MATCH(transaction_fpds.place_of_perform_county_co, '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 3, '0')
LEFT JOIN
 (SELECT code, name, fips, MAX(id) FROM state_data GROUP BY code, name, fips) AS POP_STATE_LOOKUP
 ON (POP_STATE_LOOKUP.code = transaction_fpds.place_of_performance_state)
LEFT JOIN
 (SELECT code, name, fips, MAX(id) FROM state_data GROUP BY code, name, fips) AS RL_STATE_LOOKUP
 ON (RL_STATE_LOOKUP.code = transaction_fpds.legal_entity_state_code)
...
LEFT JOIN ref_population_county AS POP_COUNTY_POPULATION
  ON (POP_COUNTY_POPULATION.state_code = POP_STATE_LOOKUP.fips AND POP_COUNTY_POPULATION.county_number = LPAD(CAST(CAST((REGEXP_MATCH(transaction_fpds.place_of_perform_county_co, '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 3, '0'))
...
LEFT JOIN ref_population_county AS RL_COUNTY_POPULATION
  ON (RL_COUNTY_POPULATION.state_code = RL_STATE_LOOKUP.fips AND RL_COUNTY_POPULATION.county_number = LPAD(CAST(CAST((REGEXP_MATCH(transaction_fpds.legal_entity_county_code, '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 3, '0'))
LEFT JOIN ref_population_cong_district AS POP_DISTRICT_POPULATION
  ON (POP_DISTRICT_POPULATION.state_code = POP_STATE_LOOKUP.fips AND POP_DISTRICT_POPULATION.congressional_district = LPAD(CAST(CAST((REGEXP_MATCH(transaction_fpds.place_of_performance_congr, '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 2, '0'))
LEFT JOIN ref_population_cong_district AS RL_DISTRICT_POPULATION
  ON (RL_DISTRICT_POPULATION.state_code = RL_STATE_LOOKUP.fips AND RL_DISTRICT_POPULATION.congressional_district = LPAD(CAST(CAST((REGEXP_MATCH(transaction_fpds.legal_entity_congressional, '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 2, '0'))
LEFT JOIN LATERAL (
SELECT recipient_hash, recipient_unique_id, ARRAY_AGG(recipient_level) AS recipient_levels
FROM recipient_profile
WHERE (recipient_hash = COALESCE(recipient_lookup.recipient_hash, MD5(UPPER(CASE WHEN transaction_fpds.awardee_or_recipient_uniqu IS NOT NULL THEN CONCAT('duns-', transaction_fpds.awardee_or_recipient_uniqu) ELSE CONCAT('name-', transaction_fpds.awardee_or_recipient_legal) END))::uuid)
OR recipient_unique_id = transaction_fpds.awardee_or_recipient_uniqu) and
...
ALTER MATERIALIZED VIEW IF EXISTS mv_idv_award_search RENAME TO mv_idv_award_search_old;
ALTER INDEX IF EXISTS idx_e4637983$973_id RENAME TO idx_e4637983$973_id_old;
ALTER INDEX IF EXISTS idx_e4637983$973_recipient_hash RENAME TO idx_e4637983$973_recipient_hash_old;
ALTER INDEX IF EXISTS idx_e4637983$973_recipient_unique_id RENAME TO idx_e4637983$973_recipient_unique_id_old;
ALTER INDEX IF EXISTS idx_e4637983$973_action_date RENAME TO idx_e4637983$973_action_date_old;
ALTER INDEX IF EXISTS idx_e4637983$973_funding_agency_id RENAME TO idx_e4637983$973_funding_agency_id_old;
ALTER INDEX IF EXISTS idx_e4637983$973_recipient_location_state_code RENAME TO idx_e4637983$973_recipient_location_state_code_old;
ALTER INDEX IF EXISTS idx_e4637983$973_recipient_location_county_code RENAME TO idx_e4637983$973_recipient_location_county_code_old;
ALTER INDEX IF EXISTS idx_e4637983$973_recipient_location_cong_code RENAME TO idx_e4637983$973_recipient_location_cong_code_old;


ALTER MATERIALIZED VIEW mv_idv_award_search_temp RENAME TO mv_idv_award_search;
ALTER INDEX idx_e4637983$973_id_temp RENAME TO idx_e4637983$973_id;
ALTER INDEX idx_e4637983$973_recipient_hash_temp RENAME TO idx_e4637983$973_recipient_hash;
ALTER INDEX idx_e4637983$973_recipient_unique_id_temp RENAME TO idx_e4637983$973_recipient_unique_id;
ALTER INDEX idx_e4637983$973_action_date_temp RENAME TO idx_e4637983$973_action_date;
ALTER INDEX idx_e4637983$973_funding_agency_id_temp RENAME TO idx_e4637983$973_funding_agency_id;
ALTER INDEX idx_e4637983$973_recipient_location_state_code_temp RENAME TO idx_e4637983$973_recipient_location_state_code;
ALTER INDEX idx_e4637983$973_recipient_location_county_code_temp RENAME TO idx_e4637983$973_recipient_location_county_code;
ALTER INDEX idx_e4637983$973_recipient_location_cong_code_temp RENAME TO idx_e4637983$973_recipient_location_cong_code;


ANALYZE VERBOSE mv_idv_award_search;
GRANT SELECT ON mv_idv_award_search TO readonly;
