DROP MATERIALIZED VIEW IF EXISTS mv_contract_award_search_temp CASCADE;
DROP MATERIALIZED VIEW IF EXISTS mv_contract_award_search_old CASCADE;

CREATE MATERIALIZED VIEW mv_contract_award_search_temp AS
SELECT
...
WHERE
  ...
  AND awards.type IN ('A', 'B', 'C', 'D')
ORDER BY
  awards.total_obligation DESC NULLS LAST WITH DATA;

CREATE UNIQUE INDEX idx_e4637983$af5_id_temp ON mv_contract_award_search_temp USING BTREE(award_id) WITH (fillfactor = 97);
...
CREATE INDEX idx_e4637983$af5_action_date_temp ON mv_contract_award_search_temp USING BTREE(action_date DESC NULLS LAST) WITH (fillfactor = 97);
...
CREATE INDEX idx_e4637983$af5_recipient_location_county_code_temp ON mv_contract_award_search_temp USING BTREE(recipient_location_county_code) WITH (fillfactor = 97) WHERE recipient_location_county_code IS NOT NULL;
