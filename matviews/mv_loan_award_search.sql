...
CREATE MATERIALIZED VIEW mv_loan_award_search_temp AS
SELECT
  ...
  awards.fain,
  awards.uri,
  COALESCE(awards.total_subsidy_cost, 0)::NUMERIC(23, 2) AS award_amount,
  0::NUMERIC(23, 2) as total_obligation,
  awards.description,
  obligation_to_enum(COALESCE(awards.total_subsidy_cost, 0)) AS total_obl_bin,
...
CREATE INDEX idx_e4637983$c34_recipient_hash_temp ON mv_loan_award_search_temp USING BTREE(recipient_hash) WITH (fillfactor = 97);
CREATE INDEX idx_e4637983$c34_recipient_unique_id_temp ON mv_loan_award_search_temp USING BTREE(recipient_unique_id) WITH (fillfactor = 97) WHERE recipient_unique_id IS NOT NULL;
...
CREATE INDEX idx_e4637983$c34_funding_agency_id_temp ON mv_loan_award_search_temp USING BTREE(funding_agency_id ASC NULLS LAST) WITH (fillfactor = 97) WHERE funding_agency_id IS NOT NULL;
CREATE INDEX idx_e4637983$c34_recipient_location_state_code_temp ON mv_loan_award_search_temp USING BTREE(recipient_location_state_code) WITH (fillfactor = 97) WHERE recipient_location_state_code IS NOT NULL;
...
CREATE INDEX idx_e4637983$c34_recipient_location_cong_code_temp ON mv_loan_award_search_temp USING BTREE(recipient_location_congressional_code) WITH (fillfactor = 97) WHERE recipient_location_congressional_code IS NOT NULL;
...
ANALYZE VERBOSE mv_loan_award_search;
GRANT SELECT ON mv_loan_award_search TO readonly;
