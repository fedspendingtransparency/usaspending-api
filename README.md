...
export DATA_BROKER_URL=postgres://admin:root@localhost:5435/data_broker
```
...
3. To run all USAspending tests in the docker services run
    ```shell
    docker compose run --rm -e DATA_BROKER_URL='' usaspending-test
    ```

_**NOTE**: If an env var named `DATA_BROKER_URL` is set, Broker Integration tests will attempt to be run as well. If doing so, Broker dependencies must be met (see below) or ALL tests will fail hard. Running the above command with `-e DATA_BROKER_URL=''` is a precaution to keep them excluded, unless you really want them (see below if so)._
...
1. Ensure you have built the `Broker` backend Docker image by running:
    ...
    ```
1. Ensure you have the `DATA_BROKER_URL` environment variable set, and it points to what will be a live PostgreSQL server (no database required) at the time tests are run.
...
_**NOTE**: `DATA_BROKER_URL` is set in the `docker-compose.yml` file (and could pick up `.env` values, if set)_
...
_**NOTE**: `DATA_BROKER_URL` is set in the `.envrc` file and available in the shell_
```shell
