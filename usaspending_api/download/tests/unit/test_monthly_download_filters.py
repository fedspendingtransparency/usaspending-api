...
def test_toptier_agency_abbreviation_error(mock_toptier_agency):
    ...
    try:
        ...
        assert len(err.raw_errors) == 1
        assert "Invalid abbreviation for 'awarding_toptier_agency_abbreviation': USDA" in str(err.raw_errors[0])
    else:
        raise AssertionError("No exception was raised")
...
def test_default_as_of_date(mock_toptier_agency, mock_date):
    ...
    filters = MonthlyDownloadFilters(awarding_toptier_agency_abbreviation="DOD")
