...
def test_query_paths_setup():
    for dl_type in ('transaction', 'award'):
        for file_source in ('d1', 'd2'):
            assert len(download_column_lookups.query_paths[dl_type][file_source]) > 10
...
def test_changes_in_award_path():
    ...
    for file_source in ('d1', 'd2'):
        assert len(_values_starting_with(download_column_lookups.query_paths['transaction'][file_source].values(), 'transaction__')) > 10
        assert len(_values_starting_with(download_column_lookups.query_paths['award'][file_source].values(), 'transaction__')) == 0
