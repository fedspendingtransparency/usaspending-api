...
from usaspending_api.awards.models import TransactionFPDS, TransactionDelta, TransactionNormalized
...
def monthly_download_delta_data(db, monkeypatch):
    ...
    mommy.make("awards.TransactionDelta", transaction=transaction, created_at=datetime.datetime.now())
...
def test_all_agencies(client, monthly_download_delta_data, monkeypatch):
    call_command("populate_monthly_delta_files", "--debugging_skip_deleted", "--last_date=2019-12-31")
...
def test_specific_agency(client, monthly_download_delta_data, monkeypatch):
    call_command("populate_monthly_delta_files", "--agencies=1", "--debugging_skip_deleted", "--last_date=2019-12-31")
    file_list = listdir("csv_downloads")
    formatted_date = datetime.datetime.strftime(datetime.date.today(), "%Y%m%d")
    ...
    with zipfile.ZipFile(f"FY(All)_001_Contracts_Delta_{formatted_date}.zip", "r") as zip_ref:
        ...
        assert f"FY2020_001_Contracts_Delta_{formatted_date}_1.csv" in listdir("csv_downloads")
    with open(
        os.path.normpath(f"csv_downloads/FY2020_001_Contracts_Delta_{formatted_date}_1.csv"), "r"
    ) as contract_file:
