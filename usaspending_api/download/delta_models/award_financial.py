...
AWARD_FINANCIAL_DELTA_COLUMNS = {k: v["delta"] for k, v in AWARD_FINANCIAL_COLUMNS.items()}
AWARD_FINANCIAL_POSTGRES_COLUMNS = {k: v["postgres"] for k, v in AWARD_FINANCIAL_COLUMNS.items()}

award_financial_create_sql_string = rf"""
    CREATE OR REPLACE TABLE {{DESTINATION_TABLE}} (
        {", ".join([f"{key} {val}" for key, val in AWARD_FINANCIAL_DELTA_COLUMNS.items()])}
    )
    USING DELTA
    LOCATION 's3a://{{SPARK_S3_BUCKET}}/{{DELTA_LAKE_S3_PATH}}/{{DESTINATION_DATABASE}}/{{DESTINATION_TABLE}}'
    """

award_financial_load_sql_string = rf"""
    INSERT OVERWRITE {{DESTINATION_DATABASE}}.{{DESTINATION_TABLE}} (
        {",".join(list(AWARD_FINANCIAL_COLUMNS))}
    )
    SELECT
