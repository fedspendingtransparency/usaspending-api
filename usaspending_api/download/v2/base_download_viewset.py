...
from usaspending_api.common.spark.jobs import DatabricksStrategy, DuckDBStrategy, SparkJobs
...
class BaseDownloadViewSet(APIView):
    ...
    def process_request(self, download_job: DownloadJob, request: Request, json_request: dict):
        ...
        elif settings.IS_LOCAL and (
            "account_balances" in json_request["download_types"]
            or "object_class_program_activity" in json_request["download_types"]
        ):
            self.process_account_download_in_spark(download_job=download_job)
