...
POSTGRES_DOWNLOAD_QUERY = """
    SELECT
        owning_agency_name,
        STRING_AGG(reporting_agency_name, '; ') AS reporting_agency_name,
        submission_period,
        federal_account_symbol,
        federal_account_name,
        agency_identifier_name,
        STRING_AGG(budget_function, '; ') AS budget_function,
        STRING_AGG(budget_subfunction, '; ') AS budget_subfunction,
        program_activity_code,
        program_activity_name,
        object_class_code,
        object_class_name,
