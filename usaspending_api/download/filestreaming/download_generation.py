...
def generate_download(  # noqa: PLR0912,PLR0915
    download_job: DownloadJob, origination: Optional[str] = None
) -> str | None:
        ...
        main_trace.set_attributes(
            {
                ...
                "number_of_rows": download_job.number_of_rows
                if download_job.number_of_rows is not None
                else 0,
                "number_of_columns": (
                ...
                "json_request": str(download_job.json_request)
    ...
    if not settings.IS_LOCAL:
        ...
        with SubprocessTrace(
            ...
            try:
                ...
                s3_span.set_attributes(
                    {"bucket": bucket, "region": region, "file": zip_file_path}
                )
                start_uploading = time.perf_counter()
                multipart_upload(
                    bucket, region, zip_file_path, os.path.basename(zip_file_path)
                )
                write_to_log(
...
def get_download_sources(  # noqa: PLR0912, PLR0915
    ...
    for download_type in json_request["download_types"]:
        ...
        if VALUE_MAPPINGS[download_type]["source_type"] == "award":
            ...
            if (
                award_type_codes
                & (set(contract_type_mapping.keys()) | set(idv_type_mapping.keys()))
    ...
    verify_requested_columns_available(
        tuple(download_sources), json_request.get("columns", [])
    )
...
def parse_source(  # noqa: PLR0913
    ...
    export_query = generate_export_query(
        source_query, limit, source, columns, file_format
    )
    temp_file, temp_file_path = generate_export_query_temp_file(
        export_query, download_job
    )
    ...
    try:
        ...
        write_to_log(
            message=f"Running {source.file_name} using psql", download_job=download_job
        )
        psql_process.start()
        ...
        write_to_log(
            message="Counting rows in delimited text file", download_job=download_job
        )
        try:
            number_of_rows = count_rows_in_delimited_file(
...
def build_data_file_name(  # noqa: PLR0912
    source: DownloadSource, download_job: DownloadJob, piid: str, assistance_id: str
) -> str:
    ...
    if source.is_for_idv or source.is_for_contract:
        ...
        if request.get("limit") or (
            request.get("request_type") == "disaster"
            and source.source_type in ("elasticsearch_awards", "sub_awards")
        ):
            agency = ""
        ...
        if request.get("request_type") == "disaster":
            ...
            current_fiscal_period = (
                f"FY{account_filters['latest_fiscal_year']}"
                f"P{str(account_filters['latest_fiscal_period']).zfill(2)}"
            )
...
def split_and_zip_data_files(
    ...
    try:
        ...
        write_to_log(
            message="Beginning zipping and compression", download_job=download_job
        )
        log_time = time.perf_counter()
...
def wait_for_process(
    process: multiprocessing.Process, start_time: float, download_job: DownloadJob
) -> float:
...
def generate_export_query_temp_file(
    ...
    (temp_sql_file, temp_sql_file_path) = tempfile.mkstemp(
        prefix="bd_sql_", dir=dir_name
    )
...
def apply_annotations_to_sql(
    ...
    selects_list = [
        val for val in select_statements if re.search(DIRECT_SELECT_QUERY_REGEX, val)
    ]
    ...
    if group_by_to_replace:
        ...
        for idx, _ in group_by_to_replace:
            ...
            second_half_query = second_half_query.replace(
                f" {idx}", f" {{idx_{idx}}}", 1
            )
    ...
    values_list = [
        f'{deriv_dict[alias] if alias in deriv_dict else selects_list.pop(0)} AS "{alias}"'
        for alias in aliases
    ]

    sql = raw_query.replace(
        _top_level_split(raw_query, "FROM")[0], f"SELECT {', '.join(values_list)} ", 1
    )
...
def execute_psql(
    temp_sql_file_path: str, source_path: str, download_job: DownloadJob
) -> None:
    ...
    with subprocess_trace as span:
        ...
        try:
            ...
            if download_job and not download_job.monthly_download:
                # Terminating the process isn't guaranteed to end the DB statement; add timeout to client connection
                temp_env["PGOPTIONS"] = (
            ...
            cat_command = subprocess.Popen(
                ["cat", temp_sql_file_path], stdout=subprocess.PIPE
            )
            ...
            write_to_log(
                message=f"Faulty SQL: {sql}", is_error=True, download_job=download_job
            )
            raise e
...
def fail_download(
    download_job: DownloadJob, exception: Exception, message: str
) -> None:
