...
class DefaultConfig(BaseSettings):
    ...
    Attributes:
        ...
        DATA_BROKER_URL: (optional) Full URL to Broker DB that can be used to override the URL-by-parts
    ...
    DATA_BROKER_URL: str = None  # FACTORY_PROVIDED_VALUE. See its root validator-factory below
    ...
    # Pydantic returns a classmethod for its validators, so the cls param is correct
    @root_validator
    def _DATA_BROKER_URL_and_parts_factory(cls, values):
        """A root validator to backfill DATA_BROKER_URL and BROKER_DB_* part config vars and validate
