...
UPDATE_SQL = """
    ...
    where
        ...
        fpds.transaction_id between %(min_id)s and %(max_id)s;
    ...
    where
        tn.id between %(min_id)s and %(max_id)s and
        fabs.transaction_id between %(min_id)s and %(max_id)s;
...
def process_chunk():
    ...
    execute("Update transactions {} through {}".format(chunk_min_id, chunk_max_id), sql)
