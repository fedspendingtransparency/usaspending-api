...
def load_psc(fullpath):
    ...
    try:
        ...
        for current_row, row in enumerate(ws.rows):
            if not row[0].value or row[0].value == "PSC CODE":
                continue  # skip lines without codes
            psc_code = str(row[0].value)
            psc_length = len(psc_code.strip())
            ...
            try:
                ...
                if psc_end_date:
                    if psc.end_date and psc_end_date > psc.end_date:
                        psc.end_date = psc_end_date
                if psc_start_date:
                    if psc.start_date and psc_start_date > psc.start_date:
                        psc.start_date = psc_start_date
            except PSC.DoesNotExist:
                psc = PSC(code=psc_code)
            ...
            #             psc, created = PSC.objects.get_or_create(code=psc_code, start_date__isnull=True, end_date__isnull=True)
            # elif psc_start_date:
