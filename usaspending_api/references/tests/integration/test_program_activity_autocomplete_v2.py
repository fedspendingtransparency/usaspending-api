...
def test_program_activity_autocomplete_success(client, program_activity_data):
    ...
    resp = client.post(
        "/api/v2/autocomplete/program_activity/", content_type="application/json", data=json.dumps({"search_text": "0003"})
    )
    ...
    resp = client.post(
        "/api/v2/autocomplete/program_activity/", content_type="application/json", data=json.dumps({"search_text": "EA"})
    )
    assert resp.status_code == status.HTTP_200_OK
