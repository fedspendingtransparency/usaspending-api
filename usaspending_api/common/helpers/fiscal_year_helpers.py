...
def calculate_last_completed_fiscal_quarter(fiscal_year, as_of_date=now()):
    """
    ...
    rows = DABSSubmissionWindowSchedule.objects.all()
    print(rows.count())
    rows = DABSSubmissionWindowSchedule.objects.filter(
        Q(submission_reveal_date__lte=as_of_date) & Q(submission_fiscal_year=fiscal_year)
    )
    print(rows.count())
    for row in rows:
        print(row)
    row = rows.aggregate(Max("submission_fiscal_quarter"))
    print(row)
