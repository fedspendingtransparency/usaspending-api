...
class AbstractToCSVStrategy(ABC):
    ...
    def download_to_csv(  # noqa: PLR0913
        ...
        skip_s3_delete: bool = False,
        skip_zip_file: bool = False,
    ) -> CSVDownloadMetadata:
        """
        Args:
            ...
            source_df: A pyspark DataFrame that contains the data to be downloaded
                Defaults to None.
            skip_s3_delete: If the generated download files should be removed from S3
            skip_zip_file: If the generated download files should be zipped
...
class SparkToCSVStrategy(AbstractToCSVStrategy):
    ...
    def download_to_csv(  # noqa: PLR0913
        ...
        try:
            ...
            self.spark = spark or get_active_spark_session()
            self.spark_created_by_command = False
            if not self.spark:
                self._logger.info("***** SPARK SESSION STARTED *****")
