...
class NaicsCodes(_Filter, HierarchicalFilter):
    ...
    @staticmethod
    def code_is_parent_of(code, other) -> bool:  # noqa: ANN001
        return len(str(other)) == len(str(code)) + 2 and other[: len(str(code))] == str(code)

    @staticmethod
    def node(code, positive, positive_naics, negative_naics) -> NaicsNode:  # noqa: ANN001
        return NaicsNode(code, positive, positive_naics, negative_naics)

    @staticmethod
    def naics_code_to_naics_code_path(code) -> list:  # noqa: ANN001
