...
class TransactionSearch(AbstractSearch):
    ...
    def agency_cols(self) -> list[Column]:
        return [
            sf.coalesce(
                self.transaction_fabs.awarding_agency_code,
                self.transaction_fpds.awarding_agency_code,
            ).alias("awarding_agency_code"),
            self.awarding_toptier_agency.awarding_toptier_agency_name,
            sf.coalesce(
                self.transaction_fabs.awarding_agency_name,
                self.transaction_fpds.awarding_agency_name,
            ).alias("awarding_toptier_agency_name_raw"),
            sf.coalesce(
                self.transaction_fabs.funding_agency_code,
                self.transaction_fpds.funding_agency_code,
            ).alias("funding_agency_code"),
            self.funding_toptier_agency.funding_toptier_agency_name,
            sf.coalesce(
                self.transaction_fabs.funding_agency_name,
                self.transaction_fpds.funding_agency_name,
            ).alias("funding_toptier_agency_name_raw"),
            sf.coalesce(
                self.transaction_fabs.awarding_sub_tier_agency_c,
                self.transaction_fpds.awarding_sub_tier_agency_c,
            ).alias("awarding_sub_tier_agency_c"),
            self.awarding_subtier_agency.awarding_subtier_agency_name,
            sf.coalesce(
                self.transaction_fabs.awarding_sub_tier_agency_n,
                self.transaction_fpds.awarding_sub_tier_agency_n,
            ).alias("awarding_subtier_agency_name_raw"),
            sf.coalesce(
                self.transaction_fabs.funding_sub_tier_agency_co,
                self.transaction_fpds.funding_sub_tier_agency_co,
            ).alias("funding_sub_tier_agency_co"),
            self.funding_subtier_agency.funding_subtier_agency_name,
            sf.coalesce(
                self.transaction_fabs.funding_sub_tier_agency_na,
                self.transaction_fpds.funding_sub_tier_agency_na,
            ...
            sf.coalesce(
                self.transaction_fabs.awarding_office_code,
                self.transaction_fpds.awarding_office_code,
            ).alias("awarding_office_code"),
            sf.coalesce(
            ...
            sf.coalesce(
                self.transaction_fabs.funding_office_code,
                self.transaction_fpds.funding_office_code,
            ).alias("funding_office_code"),
            sf.coalesce(
    ...
    def generated_parent_recipient_hash(self) -> Column:
        return hash_col(
            sf.when(
                sf.coalesce(
                    self.transaction_fpds.ultimate_parent_uei,
                    self.transaction_fabs.ultimate_parent_uei,
                ).isNotNull(),
                sf.concat(
                    sf.lit("uei-"),
                    sf.coalesce(
        ...
        return [
            sf.coalesce(
                ...
                hash_col(
                    sf.when(
                        sf.coalesce(
                            self.transaction_fpds.awardee_or_recipient_uei,
                            self.transaction_fabs.uei,
                        ).isNotNull(),
                        sf.concat(
                            sf.lit("uei-"),
                            sf.coalesce(
    ...
    def recipient_location_cols(self) -> list[Column]:
        return [
            ...
            sf.coalesce(
                self.transaction_fpds.legal_entity_country_name,
                self.transaction_fabs.legal_entity_country_name,
            ).alias("recipient_location_country_name"),
            sf.coalesce(
                self.transaction_fpds.legal_entity_state_code,
                self.transaction_fabs.legal_entity_state_code,
            ).alias("recipient_location_state_code"),
            sf.coalesce(
                self.transaction_fpds.legal_entity_state_descrip,
                self.transaction_fabs.legal_entity_state_name,
            ...
            sf.coalesce(
                self.transaction_fpds.legal_entity_county_name,
                self.transaction_fabs.legal_entity_county_name,
            ...
            sf.coalesce(
                self.transaction_fpds.legal_entity_zip5,
                self.transaction_fabs.legal_entity_zip5,
            ).alias("recipient_location_zip5"),
            self.transaction_fpds.legal_entity_zip4,
            sf.coalesce(
                self.transaction_fpds.legal_entity_zip_last4,
                self.transaction_fabs.legal_entity_zip_last4,
            ).alias("legal_entity_zip_last4"),
            self.transaction_fabs.legal_entity_city_code,
            sf.rtrim(
                sf.coalesce(
                    self.transaction_fpds.legal_entity_city_name,
                    self.transaction_fabs.legal_entity_city_name,
                )
            ).alias("recipient_location_city_name"),
            sf.coalesce(
                self.transaction_fpds.legal_entity_address_line1,
                self.transaction_fabs.legal_entity_address_line1,
            ).alias("legal_entity_address_line1"),
            sf.coalesce(
                self.transaction_fpds.legal_entity_address_line2,
                self.transaction_fabs.legal_entity_address_line2,
            ).alias("legal_entity_address_line2"),
            sf.coalesce(
                self.transaction_fpds.legal_entity_address_line3,
                self.transaction_fabs.legal_entity_address_line3,
    ...
    def place_of_performance_cols(self) -> list[Column]:
        return [
            ...
            sf.coalesce(
                self.transaction_fpds.place_of_perf_country_desc,
                self.transaction_fabs.place_of_perform_country_n,
            ...
            sf.coalesce(
                self.transaction_fpds.place_of_perfor_state_desc,
                self.transaction_fabs.place_of_perform_state_nam,
            ...
            sf.coalesce(
                self.transaction_fpds.place_of_perform_county_na,
                self.transaction_fabs.place_of_perform_county_na,
            ...
            sf.coalesce(
                self.transaction_fpds.place_of_performance_zip5,
                self.transaction_fabs.place_of_performance_zip5,
            ).alias("pop_zip5"),
            sf.coalesce(
                self.transaction_fpds.place_of_performance_zip4a,
                self.transaction_fabs.place_of_performance_zip4a,
            ).alias("place_of_performance_zip4a"),
            sf.coalesce(
                self.transaction_fpds.place_of_perform_zip_last4,
                self.transaction_fabs.place_of_perform_zip_last4,
    ...
    def officer_amounts_cols(self) -> list[Column]:
        return [
            sf.coalesce(
                self.transaction_fabs.officer_1_name,
                self.transaction_fpds.officer_1_name,
            ).alias("officer_1_name"),
            sf.coalesce(
                self.transaction_fabs.officer_1_amount,
                self.transaction_fpds.officer_1_amount,
            ).alias("officer_1_amount"),
            sf.coalesce(
                self.transaction_fabs.officer_2_name,
                self.transaction_fpds.officer_2_name,
            ).alias("officer_2_name"),
            sf.coalesce(
                self.transaction_fabs.officer_2_amount,
                self.transaction_fpds.officer_2_amount,
            ).alias("officer_2_amount"),
            sf.coalesce(
                self.transaction_fabs.officer_3_name,
                self.transaction_fpds.officer_3_name,
            ).alias("officer_3_name"),
            sf.coalesce(
                self.transaction_fabs.officer_3_amount,
                self.transaction_fpds.officer_3_amount,
            ).alias("officer_3_amount"),
            sf.coalesce(
                self.transaction_fabs.officer_4_name,
                self.transaction_fpds.officer_4_name,
            ).alias("officer_4_name"),
            sf.coalesce(
                self.transaction_fabs.officer_4_amount,
                self.transaction_fpds.officer_4_amount,
            ).alias("officer_4_amount"),
            sf.coalesce(
                self.transaction_fabs.officer_5_name,
                self.transaction_fpds.officer_5_name,
            ).alias("officer_5_name"),
            sf.coalesce(
                self.transaction_fabs.officer_5_amount,
                self.transaction_fpds.officer_5_amount,
            ).alias("officer_5_amount"),
        ]
