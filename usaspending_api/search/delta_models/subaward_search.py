SUBAWARD_SEARCH_COLUMNS = {
    ...
    "sub_legal_entity_country_name2": {"delta": "STRING", "postgres": "TEXT"},
...
subaward_search_load_sql_string = fr"""
    ...
    SELECT
        ...
        bs.sub_legal_entity_country_name,
        ...
        LPAD(CAST(CAST((REGEXP_MATCH(UPPER(bs.sub_legal_entity_congressional), '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 2, '0') AS sub_legal_entity_congressional,
        ...
        LPAD(CAST(CAST((REGEXP_MATCH(UPPER(bs.sub_place_of_perform_congressio), '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 2, '0') AS sub_place_of_perform_congressio,
        ...
        rcc.country_name AS sub_legal_entity_country_name2,
        LPAD(CAST(CAST((REGEXP_MATCH(rec.county_numeric, '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 3, '0') AS sub_legal_entity_county_code,
        ...
        LPAD(CAST(CAST((REGEXP_MATCH(pop.county_numeric, '^[A-Z]*(\d+)(?:\.\d+)?$'))[1] AS smallint) AS text), 3, '0') AS sub_place_of_perform_county_code,
