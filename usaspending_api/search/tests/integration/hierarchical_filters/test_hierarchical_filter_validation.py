...
class TestValidateComplexity:
    ...
    def test_accepts_max_allowed_codes(self):
        ...
        require = [[f"CODE{i}"] for i in range(60)]
        exclude = [[f"CODE{i}"] for i in range(60, 100)]  # Total = 100
    ...
    def test_rejects_too_many_codes(self):
        ...
        require = [[f"CODE{i}"] for i in range(60)]
        exclude = [[f"CODE{i}"] for i in range(60, 101)]  # Total = 101

        with pytest.raises(ValueError) as exc_info:
            MockHierarchicalFilter._validate_complexity(require, exclude)

        assert "Total codes (101) exceeds limit (100)" in str(exc_info.value)
    ...
    def test_validates_combined_require_and_exclude(self):
        ...
        require = [[f"REQ{i}"] for i in range(60)]
        exclude = [[f"EXC{i}"] for i in range(50)]  # Total = 110

        with pytest.raises(ValueError) as exc_info:
            MockHierarchicalFilter._validate_complexity(require, exclude)

        assert "Total codes (110) exceeds limit (100)" in str(exc_info.value)
    ...
    def test_boundary_exactly_at_limits(self):
        ...
        # Add more codes to reach 100 total (but keep them shallow)
        require.extend([[f"X{i}"] for i in range(90)])
        exclude = []
...
class TestValidateComplexityErrorMessages:
    ...
    def test_total_codes_error_shows_actual_and_limit(self):
        ...
        require = [[f"CODE{i}"] for i in range(101)]
        exclude = []
        ...
        assert "101" in error_msg  # Actual count
