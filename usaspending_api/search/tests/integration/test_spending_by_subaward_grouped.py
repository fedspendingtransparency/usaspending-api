...
def test_spending_by_subaward_grouped_success(client, monkeypatch, elasticsearch_subaward_index, subaward_grouped_data_fixture):
    ...
    resp = client.post(
        ...
        data=json.dumps(
            {"page": 1, "limit": 2, "sort": "award_id", "filters": {}}
        ),
    )
...
@pytest.mark.django_db
def test_spending_by_subaward_grouped_award_type_filter(client, monkeypatch, elasticsearch_subaward_index, elasticsearch_award_index, subaward_grouped_data_fixture):
    ...
    resp = client.post(
        ...
        content_type="application/json",
        data=json.dumps(
            {"page": 1, "limit": 2, "sort": "award_id", "filters": {"award_type_codes": ["A"]}}
        ),
    )
    assert resp.status_code == status.HTTP_200_OK
    assert resp.json()["page_metadata"]["page"] == 1
