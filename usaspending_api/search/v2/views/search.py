...
class SpendingByAwardVisualizationViewSet(APIView):
    ...
    def post(self, request):
        ...
        if sort:
            if subawards:
                if set(filters["award_type_codes"]) <= set(list(contract_type_mapping) + list(idv_type_mapping)):  # Subaward contracts
                    ...
                    msg = 'Award Type codes limited for Subawards. Only contracts {}, IDVs {}, or grants {} are available'
                    msg = msg.format(list(contract_type_mapping.keys()), list(idv_type_mapping.keys()), list(grant_type_mapping.keys()))
