...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('submission_id', nargs=1,
                            help='a data broker submission id', type=int)

    def handle(self, *args, **options):
        # This will throw an exception and exit the command if the id doesn't exist
        try:
            submission = SubmissionAttributes.objects.get(
                broker_submission_id=options["submission_id"][0])
        except ObjectDoesNotExist:
            self.logger.error("Submission with broker id {} not found".format(
        ...
        logger.info(
            ...
            "quarter_format {}\nupdate_date {}".format(
                submission, submission.reporting_period_start,
                submission.reporting_period_end,
                submission.quarter_format_flag, submission.update_date))
        logger.info("\nModel counts:")
