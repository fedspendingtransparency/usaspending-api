    ...
    for key, value in filters.items():
        ...
        if _type == 'recipient' or _type == 'award' or _type == 'award_category' or _type == 'agency_sub':
            ...
            # award, award_category - DONE
                ...
                if or_alt_set:
                    ...
                    or_alt_set = alt_set.filter(award=value)
                if or_alt_set is not None:
                    alt_set &= or_alt_set
            ...
            # agency_type - DONE
                ...
                try:
                    if or_alt_set:
                        or_alt_set |= or_alt_set.filter(treasury_account__awarding_toptier_agency=value)
                    else:
                        or_alt_set = alt_set.filter(treasury_account__awarding_toptier_agency=value)
                    if or_alt_set is not None:
                        alt_set &= or_alt_set
                except urllib.error.HTTPError:
            ...
            # agency_sub - DONE
                ...
                try:
                    if or_alt_set:
                        or_alt_set |= or_alt_set.filter(award__awarding_agency__subtier_agency=value)
                    else:
                        or_alt_set = alt_set.filter(award__awarding_agency__subtier_agency=value)
                    if or_alt_set is not None:
                        alt_set &= or_alt_set
                except urllib.error.HTTPError:
        ...
        # All other _type
            ...
            # award, award_category - DONE
                ...
                if or_queryset:
                    or_queryset |= or_queryset.filter(treasury_account__in=alt_set.filter(
                        award=value).values_list('treasury_account_id', flat=True))
                else:
                    or_queryset = queryset.filter(treasury_account__in=alt_set.filter(
                        award=value).values_list('treasury_account_id', flat=True))
                if or_queryset is not None:
            ...
            # agency_type - DONE
                ...
                try:
                    if or_queryset:
                        or_queryset |= or_queryset.filter(treasury_account__awarding_toptier_agency=value)
                    else:
                        or_queryset = queryset.filter(treasury_account__awarding_toptier_agency=value)
                    if or_queryset is not None:
                        queryset &= or_queryset
                except urllib.error.HTTPError:
                    raise InvalidParameterException('Invalid top tier agency ID: ' + value + ' does not exist.')
            ...
            # agency_sub - DONE
                ...
                try:
                    if or_queryset:
                        or_queryset |= or_queryset.filter(treasury_account__in=alt_set.filter(
                            award__in=Award.objects.all().filter(
                                awarding_agency__subtier_agency=value)).values_list('treasury_account_id', flat=True))
                    else:
                        or_queryset = queryset.filter(treasury_account__in=alt_set.filter(
                            award__in=Award.objects.all().filter(
                                awarding_agency__subtier_agency=value)).values_list('treasury_account_id', flat=True))
