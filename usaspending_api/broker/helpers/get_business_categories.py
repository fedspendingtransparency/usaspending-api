...
from usaspending_api.broker.helpers.build_business_categories_boolean_dict import fpds_columns as fpds_business_categories
...
def get_business_categories_fpds(row):
    ...
    def row_get(row, col_name):
        ...
        if col_name in fpds_business_categories:
            except_val = False
        else:
            except_val = None
    ...
    # Foreign-Owned Business Not Incorporated in the U.S.
    if (
        row_get(row, "domestic_or_foreign_entity") == "D"
        or row_get(row, "foreign_owned_and_located") is True
    ):
        business_category_set.add("foreign_owned")
    ...
    if (
        row_get(row, "us_tribal_government") is True
        or row_get(row, "indian_tribe_federally_rec") is True
    ):
        business_category_set.add("indian_native_american_tribal_government")
