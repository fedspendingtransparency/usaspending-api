...
def get_fabs_records_to_delete(start_datetime, end_datetime):
    """
    ...
    logger.info("CSV data from {} to now".format(start_datetime))

    filtered_csv_list = [
        x
        for x in bucket_objects
        if (x.key.endswith(".csv") and not x.key.startswith("staging") and x.last_modified >= start_datetime)
    ]
    ...
    for obj in filtered_csv_list:
        ...
        if "detached_award_proc_unique" in data:
            ...
            logger.info("  [Missing valid col] in {}".format(obj.key))
