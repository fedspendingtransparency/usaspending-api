...
class Command(BaseCommand):
    ...
    def finalize_zip_contents(self):
        ...
        file_description_path = save_file_description(
            str(self.zip_file_path.parent), str(self.zip_file_path), file_description
        )
    ...
    def download_to_csv(self, sql_filepath, destination_path):
        ...
        export_query = r"\COPY ({}) TO STDOUT {}".format(sql_filepath.read_text(), options)
        try:
            ...
            psql_process = multiprocessing.Process(
                target=execute_psql, args=(temp_file_path, str(destination_path), None)
            )
            psql_process.start()
            ...
            logger.info(f"Counting rows in delimited text file {str(destination_path)}")
            try:
                count = count_rows_in_delimited_file(filename=str(destination_path), has_header=True, delimiter=delim)
            ...
            zip_process = multiprocessing.Process(
                ...
                args=(str(self.zip_file_path), temp_file_path, str(destination_path), self.file_format, None),
            )
            zip_process.start()
