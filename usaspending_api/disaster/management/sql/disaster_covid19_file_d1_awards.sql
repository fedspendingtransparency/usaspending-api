...
SELECT
    ...
    covid_faba_contract_awards_agg.disaster_emergency_fund_codes AS disaster_emergency_fund_codes,
    (
        covid_faba_contract_awards_agg.gross_outlay_amount_by_award_cpe
        + covid_faba_contract_awards_agg.ussgl487200_down_adj_pri_ppaid_undel_orders_oblig_refund_cpe
        + covid_faba_contract_awards_agg.ussgl497200_down_adj_pri_paid_deliv_orders_oblig_refund_cpe
    ) AS "outlayed_amount_funded_by_COVID-19_supplementals",
    covid_faba_contract_awards_agg.transaction_obligated_amount AS "obligated_amount_funded_by_COVID-19_supplementals",
    ...
    faba_contract_awards_agg.treasury_accounts_funding_this_award AS treasury_accounts_funding_this_award,
    faba_contract_awards_agg.federal_accounts_funding_this_award AS federal_accounts_funding_this_award,
    faba_contract_awards_agg.object_classes_funding_this_award AS object_classes_funding_this_award,
    faba_contract_awards_agg.program_activities_funding_this_award AS program_activities_funding_this_award,
...
FROM rpt.award_search /* constrained with is_fpds = TRUE in the covid_faba_contract_awards_agg subquery that it is INNER JOINed to */
...
INNER JOIN rpt.transaction_search AS earliest_transaction
    ON (award_search.earliest_transaction_id = earliest_transaction.transaction_id)
INNER JOIN (
    SELECT
        awd.award_id,
...
) AS covid_faba_contract_awards_agg
    ON award_search.award_id = covid_faba_contract_awards_agg.award_id,
LATERAL (
    SELECT
        awd.award_id,
