...
SELECT
    ...
    CASE
        ...
        ELSE
            CONCAT('FY', EXTRACT(YEAR FROM (financial_accounts_by_program_activity_object_class.reporting_period_end) + INTERVAL 3 months), 'P', LPAD(STRING(EXTRACT(MONTH FROM (financial_accounts_by_program_activity_object_class.reporting_period_end) + INTERVAL 3 months)), 2, '0'))
    ...
    TO_DATE((MAX(submission_attributes.published_date))) AS last_modified_date
FROM global_temp.financial_accounts_by_program_activity_object_class
LEFT OUTER JOIN global_temp.treasury_appropriation_account ON (financial_accounts_by_program_activity_object_class.treasury_account_id = treasury_appropriation_account.treasury_account_identifier)
INNER JOIN global_temp.federal_account ON (treasury_appropriation_account.federal_account_id = federal_account.id)
INNER JOIN global_temp.submission_attributes ON (financial_accounts_by_program_activity_object_class.submission_id = submission_attributes.submission_id)
INNER JOIN global_temp.disaster_emergency_fund_code ON (financial_accounts_by_program_activity_object_class.disaster_emergency_fund_code = disaster_emergency_fund_code.code)
INNER JOIN global_temp.ref_program_activity ON (financial_accounts_by_program_activity_object_class.program_activity_id = ref_program_activity.id)
INNER JOIN global_temp.object_class ON (financial_accounts_by_program_activity_object_class.object_class_id = object_class.id)
LEFT OUTER JOIN global_temp.toptier_agency ON (treasury_appropriation_account.funding_toptier_agency_id = toptier_agency.toptier_agency_id)
LEFT OUTER JOIN global_temp.cgac AS cgac_aid ON (treasury_appropriation_account.agency_id = cgac_aid.cgac_code)
LEFT OUTER JOIN global_temp.cgac AS cgac_ata ON (treasury_appropriation_account.allocation_transfer_agency_id = cgac_ata.cgac_code)
WHERE (
...
GROUP BY
    ...
    financial_accounts_by_program_activity_object_class.obligations_incurred_by_program_object_class_cpe,
    financial_accounts_by_program_activity_object_class.obligations_undelivered_orders_unpaid_total_cpe,
    financial_accounts_by_program_activity_object_class.obligations_undelivered_orders_unpaid_total_fyb,
    financial_accounts_by_program_activity_object_class.ussgl480100_undelivered_orders_obligations_unpaid_cpe,
    ...
    financial_accounts_by_program_activity_object_class.gross_outlays_delivered_orders_paid_total_cpe,
    financial_accounts_by_program_activity_object_class.gross_outlays_delivered_orders_paid_total_fyb,
    financial_accounts_by_program_activity_object_class.ussgl490200_delivered_orders_obligations_paid_cpe,
    financial_accounts_by_program_activity_object_class.ussgl490800_authority_outlayed_not_yet_disbursed_cpe,
    financial_accounts_by_program_activity_object_class.ussgl490800_authority_outlayed_not_yet_disbursed_fyb,
    financial_accounts_by_program_activity_object_class.ussgl498200_upward_adjust_pri_deliv_orders_oblig_paid_cpe,
    financial_accounts_by_program_activity_object_class.deobligations_recoveries_refund_pri_program_object_class_cpe,
    financial_accounts_by_program_activity_object_class.ussgl487100_down_adj_pri_unpaid_undel_orders_oblig_recov_cpe,
    financial_accounts_by_program_activity_object_class.ussgl497100_down_adj_pri_unpaid_deliv_orders_oblig_recov_cpe,
    financial_accounts_by_program_activity_object_class.ussgl487200_down_adj_pri_ppaid_undel_orders_oblig_refund_cpe,
    financial_accounts_by_program_activity_object_class.ussgl497200_down_adj_pri_paid_deliv_orders_oblig_refund_cpe,
    financial_accounts_by_program_activity_object_class.ussgl483100_undelivered_orders_oblig_transferred_unpaid_cpe,
    financial_accounts_by_program_activity_object_class.ussgl493100_delivered_orders_oblig_transferred_unpaid_cpe,
    financial_accounts_by_program_activity_object_class.ussgl483200_undeliv_orders_oblig_transferred_prepaid_adv_cpe
"""
