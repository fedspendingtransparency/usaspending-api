...
SELECT
    ...
    (SELECT CONCAT_WS(';', COLLECT_SET(CONCAT(U2.object_class, ':', U2.object_class_name))) FROM rpt.award_search U0 LEFT OUTER JOIN int.financial_accounts_by_awards U1 ON (U0.award_id = U1.award_id) INNER JOIN global_temp.object_class U2 ON (U1.global_temp.object_classid = U2.id) WHERE U0.award_id = (subaward_search.award_id) and U1.object_class_id is not null GROUP BY U0.award_id) AS prime_award_object_classes_funding_this_award,
