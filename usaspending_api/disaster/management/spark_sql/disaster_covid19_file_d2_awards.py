disaster_covid19_file_d2_awards_sql = """
SELECT
    ...
    array_join(transform(award_search.cfdas, j -> concat(get_json_object(j, '$.cfda_number'), ': ', get_json_object(j, '$.cfda_program_title'))), '; ') AS cfda_numbers_and_titles,
...
INNER JOIN (
    SELECT
        faba.award_id,
        array_join(array_sort(collect_set(CONCAT(disaster_emergency_fund_code, ': ', public_law))), ';') AS disaster_emergency_funds,
