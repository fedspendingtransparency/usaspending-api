...
class Command(BaseCommand):
    ...
    latest submission window when that window's submissions are revealed. Elasticsearch calculations for
    an award's Covid values do not include data from a submission until its submission window's reveal
    date has been passed. Because incremental loads in Elasticsearch compare the award's update_date field
    to a rolling date when determining whether it should be recalculated, the data will not automatically
    be refreshed after a submission window's reveal date. This command updates the update_date field on
    awards modified by the latest revealed submission so the Elasticsearch incremental load will include
    those awards. The command uses the external_data_load_date table to make sure each submission window
