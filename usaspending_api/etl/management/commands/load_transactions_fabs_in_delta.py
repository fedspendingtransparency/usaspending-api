...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        with self.prepare_spark():
            ...
            table_exists = self.spark._jsparkSession.catalog().tableExists(f"int.transaction_fabs")
            if not table_exists:
                raise Exception(f"Table: int.transaction_fabs does not exist.")

            logger.info(f"Running delete SQL for 'transaction_fabs' ETL")
            ...
            logger.info(f"Running UPSERT SQL for 'transaction_fabs' ETL")
            self.spark.sql(self.transaction_fabs_fpds_merge_into_sql())
            update_last_load_date("transaction_fabs", next_last_load)
    ...
    def delete_records_sql(self):
        ...
        id_col = "afa_generated_unique"
        subquery = f"""
            SELECT transaction_fabs.{id_col} AS id_to_remove
            FROM int.transaction_fabs LEFT JOIN raw.published_fabs ON (
                transaction_fabs.{id_col} = published_fabs.{id_col}
            )
            WHERE transaction_fabs.{id_col} IS NOT NULL AND published_fabs.{id_col} IS NULL
        """
        ...
        sql = f"""
            ...
            ON transaction_fabs.{id_col} = deleted_records.id_to_remove
            WHEN MATCHED
            THEN DELETE
        """
    ...
    def transaction_fabs_fpds_merge_into_sql(self):
        ...
        sql = f"""
            MERGE INTO int.transaction_fabs AS silver_table
            USING (
                {self.source_subquery_sql()}
            ) AS source_subquery
            ON silver_table.afa_generated_unique = source_subquery.afa_generated_unique
            WHEN MATCHED
                THEN UPDATE SET
                    {", ".join(set_cols)}
            WHEN NOT MATCHED
