...
from usaspending_api.common.elasticsearch.elasticsearch_sql_helpers import ensure_award_etl_view_exists
...
class Command(BaseCommand):
    """ETL script for indexing award data into Elasticsearch
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help="Processes awards updated on or after the UTC date/time provided. yyyy-mm-dd hh:mm:ss is always "
...
def process_cli_parameters(options: dict, es_client) -> None:
    ...
    if config["create_new_index"] and not config["index_name"]:
        ...
        #      - When all award records in the USAspending SQL database were updated.
        #   And keep it timezone-award for S3
        config["starting_date"] = get_last_load_date("es_awards", default=default_datetime)
    ...
    if config["is_incremental_load"]:
        if config["index_name"]:
            ...
            printf({"msg": msg.format(settings.ES_AWARDS_WRITE_ALIAS)})
        config["index_name"] = settings.ES_AWARDS_WRITE_ALIAS
        if not es_client.cat.aliases(name=settings.ES_AWARDS_WRITE_ALIAS):
            printf({"msg": "Fatal error: write alias '{}' is missing".format(settings.ES_AWARDS_WRITE_ALIAS)})
            raise SystemExit(1)
    else:
