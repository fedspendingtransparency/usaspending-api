...
from usaspending_api.etl.tests.data.delta_model_for_test import (
    TEST_TABLE_POSTGRES,
    TEST_TABLE_SPEC,
    TestModel,
)
...
def equal_datasets(
    ...
    schema_type_converters = {
        ...
        "ARRAY<STRING>": _handle_string_cast,
    }
    if custom_schema:
        for schema_change in custom_schema.split(", "):
            col, new_col_type = (
                schema_change.split()[0].strip(),
                schema_change.split()[1].strip(),
            )
...
# TODO: Refactor and remove the "noqa" for PLR0912
def verify_delta_table_loaded_to_delta(  # noqa: PLR0912
    spark: SparkSession,
    delta_table_name: str,
    s3_bucket: str,
    alt_db: str = None,
...
def test_load_table_to_delta_timezone_aware(
    ...
    monkeypatch.setattr(
        "usaspending_api.etl.management.commands.load_table_to_delta.TABLE_SPEC",
        TABLE_SPEC,
    )
...
def test_incremental_load_table_to_delta_for_award_search(
    ...
    call_command(
        ...
        "--destination-table=award_search",
        "--alt-db=int",
    )
...
def test_incremental_load_table_to_delta_for_transaction_search(
    ...
    call_command(
        "create_delta_table",
        f"--spark-s3-bucket={s3_unittest_data_bucket}",
        "--destination-table=transaction_search",
        "--alt-db=int",
    )
...
def test_load_table_to_delta_for_summary_state_view(
    ...
    # We now want to load the award_search table that we created above along with other tables needed to create
    # award_search. Then create the summary_state_view table and populate it using the load_query_to_delta command.
    tables_to_load = [
        "transaction_fabs",
        "transaction_fpds",
        "transaction_normalized",
