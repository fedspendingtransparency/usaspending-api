...
from usaspending_api.common.etl.spark import USAS_RDS_REF_TABLES, _BROKER_PROXY_TABLES, create_ref_temp_views
...
def test_create_ref_temp_views(spark: SparkSession):
    ...
    for rds_ref_table in USAS_RDS_REF_TABLES:
...
def test_create_ref_broker_views(spark: SparkSession):
    # Add dummy data to each test views
    for rds_ref_table in _BROKER_PROXY_TABLES:
        baker.make(rds_ref_table)
