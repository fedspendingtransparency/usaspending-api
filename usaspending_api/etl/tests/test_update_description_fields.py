...
def test_description_fields(description_updatable_models):
    ...
    assistance_transactions = TransactionNormalized.objects.filter(assistance_data__isnull=True, contract_data__isnull=False)

    assert assistance_transactions.filter(action_type="A").first().action_type_description == daims_maps["TransactionNormalized.action_type_map"]["case_2_map"]["A"]
    assert assistance_transactions.filter(action_type="B").first().action_type_description == daims_maps["TransactionNormalized.action_type_map"]["case_2_map"]["B"]
    assert assistance_transactions.filter(action_type="C").first().action_type_description == daims_maps["TransactionNormalized.action_type_map"]["case_2_map"]["C"]
    assert assistance_transactions.filter(action_type="D").first().action_type_description == daims_maps["TransactionNormalized.action_type_map"]["case_2_map"]["D"]

    assert contract_transactions.filter(action_type="A").first().action_type_description == daims_maps["TransactionNormalized.action_type_map"]["case_1_map"]["A"]
    assert contract_transactions.filter(action_type="B").first().action_type_description == daims_maps["TransactionNormalized.action_type_map"]["case_1_map"]["B"]
    assert contract_transactions.filter(action_type="C").first().action_type_description == daims_maps["TransactionNormalized.action_type_map"]["case_1_map"]["C"]
    assert contract_transactions.filter(action_type="D").first().action_type_description == daims_maps["TransactionNormalized.action_type_map"]["case_1_map"]["D"]
