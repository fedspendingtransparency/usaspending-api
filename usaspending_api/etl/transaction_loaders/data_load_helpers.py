...
def get_deleted_fpds_data_from_s3(date):
    ...
    if settings.IS_LOCAL:
        ...
        if not (aws_region and DELETED_TRANSACTION_JOURNAL_S3_BUCKET):
            raise Exception("Missing required environment variables: USASPENDING_AWS_REGION, DELETED_TRANSACTION_JOURNAL_S3_BUCKET")

        s3client = boto3.client("s3", region_name=aws_region)
        s3resource = boto3.resource("s3", region_name=aws_region)
        s3_bucket = s3resource.Bucket(DELETED_TRANSACTION_JOURNAL_S3_BUCKET)
        ...
        for item in file_list:
            ...
            if (
                ...
                s3_item = s3client.get_object(Bucket=DELETED_TRANSACTION_JOURNAL_S3_BUCKET, Key=item)
