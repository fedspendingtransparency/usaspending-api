...
class DeltaTransactionLoader(ABC):
    ...
    def __init__(self, etl_level: Literal["fabs", "fpds", "normalized"], spark_s3_bucket: str):
        ...
        match self.etl_level:
            ...
            case _:
                raise ValueError("Invalid etl level")

    def load_transactions(self):
        with self.prepare_spark():
            if not self.spark._jsparkSession.catalog().tableExists(f"int.{self.etl_level}"):
            ...
            self.last_etl_date = str(datetime.utcfromtimestamp(0))
    ...
    def delete_records_sql(self):
        subquery = f"""
            SELECT transaction_{self.etl_level}.{self.id_col} AS id_to_remove
            FROM int.transaction_{self.etl_level} LEFT JOIN {self.source_table} ON (
                UPPER(transaction_{self.etl_level}.{self.id_col}) = UPPER({self.source_table}.{self.id_col})
            )
            WHERE transaction_{self.etl_level}.{self.id_col} IS NOT NULL AND {self.source_table}.{self.id_col}IS NULL
        """
        sql = f"""
            MERGE INTO int.transaction_{self.etl_level}
            USING (
                {subquery}
            ) AS deleted_records
            ON transaction_{self.etl_level}.{self.id_col} = deleted_records.id_to_remove
            WHEN MATCHED
            THEN DELETE
        """
    ...
    def source_subquery_sql(self):
        ...
        def handle_column(col: TransactionColumn, is_result_aliased=True):
            """
            Args:
        ...
        def select_columns_transaction_fabs_fpds():
            col_info = copy.copy(TRANSACTION_FPDS_COLUMN_INFO)

            select_cols = []
            for col in filter(lambda x: x.dest_name not in ["transaction_id"], col_info):
                select_cols.append(handle_column(col))

            return select_cols

        select_columns = select_columns_transaction_fabs_fpds()
        ...
        sql = f"""
            ...
            WHERE {self.source_table}.updated_at >= '{self.last_etl_date}'
        """
        return sql
