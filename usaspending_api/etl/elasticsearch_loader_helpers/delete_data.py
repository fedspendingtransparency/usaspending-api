...
def delete_awards(client: Elasticsearch, config: dict) -> int:
    """So we have to find all the awards connected to these transactions, if we can't find the awards in the
    database, then we have to delete them from es
...
def delete_transactions(client: Elasticsearch, config: dict) -> int:
    ...
    return delete_docs_by_unique_key(
        client,
        key=config["unique_key_field"],
        value_list=[*deleted_ids],
        task_id="Delete",
        index=config["query_alias_prefix"],
    )


def gather_deleted_ids(config: dict) -> Dict[str, Dict[str, datetime]]:
    """
