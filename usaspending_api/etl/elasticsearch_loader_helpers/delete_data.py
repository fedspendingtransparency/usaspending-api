...
def delete_awards(
    ...
    values_list = list(
        set(
            [v for d in deleted_award_kvs for v in d.values()]
            + [v for d in updated_awards_pre_fy2008 for v in d.values()]
        )
    )
...
def delete_transactions(
    ...
    pre_fy2008_transactions_len = len(pre_fy2008_transactions)

    if deleted_tx_keys is not None and pre_fy2008_transactions_len > 0:
        tx_keys_to_delete = list(set([*deleted_tx_keys] + pre_fy2008_transactions))
    elif deleted_tx_keys is None and pre_fy2008_transactions_len > 0:
        tx_keys_to_delete = pre_fy2008_transactions
    elif deleted_tx_keys is not None and pre_fy2008_transactions_len == 0:
        tx_keys_to_delete = [*deleted_tx_keys]
    else:
        tx_keys_to_delete = []
...
def _gather_modified_transactions_pre_fy2008(
    ...
    Returns:
        List of `generated_unique_transaction_id` values of transactions that have been updated in the past `days_delta` days
        or an empty list if there are no applicable transactions.
    """
...
def _check_awards_for_pre_fy2008(
    ...
    Returns:
        List of `generated_unique_award_id` values of awards that have been updated in the past `days_delta` days
        or an empty list.
    """
