...
def update_awards(award_tuple=None):
    ...
    if award_tuple:
        ...
        earliest_transaction_cte = str(earliest_transaction_cte).format(" WHERE award_id IN %s ")
        latest_transaction_cte = str(latest_transaction_cte).format(" WHERE award_id IN %s ")
        aggregate_transaction_cte = str(aggregate_transaction_cte).format(" WHERE award_id IN %s ")
    else:
        values = None
        earliest_transaction_cte = str(earliest_transaction_cte).format("")
        latest_transaction_cte = str(latest_transaction_cte).format("")
    ...
    sql_update = "WITH {}, {}, {}".format(earliest_transaction_cte, latest_transaction_cte, aggregate_transaction_cte)
    sql_update += (
        "UPDATE awards a "
        "SET "
...
def update_assistance_awards(award_tuple=None):
    ...
    if award_tuple:
        values = [award_tuple]
        sql_update = str(sql_update).format("AND award_id IN %s ")
    else:
        values = None
...
def update_contract_awards(award_tuple=None):
    ...
    sql_update = "WITH {}, {}, {}".format(aggregate_transaction_cte, extra_fpds_fields, executive_comp_cte)
    sql_update += (
        "UPDATE awards a "
        "SET "
