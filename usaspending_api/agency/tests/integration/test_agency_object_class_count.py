...
url = "/api/v2/agency/{code}/object_class/count/{filter}"
...
def test_object_class_count_success(client, monkeypatch, agency_account_data, helpers):
    ...
    assert resp.status_code == status.HTTP_200_OK
    assert resp.data["object_class_count"] == 0


@pytest.mark.django_db
def test_object_class_count_too_early(client, agency_account_data):
    resp = client.get(url.format(code="007", filter="?fiscal_year=2007"))
    assert resp.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


@pytest.mark.django_db
def test_object_class_count_future(client, agency_account_data):
    resp = client.get(url.format(code="007", filter=f"?fiscal_year={current_fiscal_year() + 1}"))
    assert resp.status_code == status.HTTP_422_UNPROCESSABLE_ENTITY


@pytest.mark.django_db
def test_object_class_count_specific(client, agency_account_data):
    resp = client.get(url.format(code="008", filter="?fiscal_year=2017"))
    assert resp.status_code == status.HTTP_200_OK
    assert resp.data["object_class_count"] == 1
...
@pytest.mark.django_db
def test_object_class_count_ignore_duplicates(client, agency_account_data):
