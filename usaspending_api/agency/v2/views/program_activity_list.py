...
class ProgramActivityList(AgencyBase):
    """
    ...
    def post(self, request: Request, *args: Any, **kwargs: Any) -> Response:
        results = self.get_program_activity_list()
    ...
    def get_program_activity_list(self) -> List[dict]:
        queryset_results = RefProgramActivity.objects.filter(
            ...
            financialaccountsbyprogramactivityobjectclass__obligations_incurred_by_program_object_class_cpe=0,
            financialaccountsbyprogramactivityobjectclass__gross_outlay_amount_by_program_object_class_cpe=0,
        )

        if self.filter:
            queryset_results = queryset_results.filter(program_activity_name__contains=self.filter)

        queryset_results = (
            queryset_results.values("pk")
            .annotate(
        ...
        results = queryset_results[self.pagination.lower_limit : self.pagination.upper_limit]
